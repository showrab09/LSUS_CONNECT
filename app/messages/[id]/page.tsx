  ...
  return (
    <AppLayout>
      <div className="flex-1 flex flex-col max-w-[1200px] w-full mx-auto">
        ...
        {error && conversation && (
          <div className="mx-4 mt-2 bg-red-500/20 border border-red-500/30 text-red-300 rounded-lg p-3 text-sm">{error}</div>
        )}
            ...
            return (
              <div key={message.id} className={`flex ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[70%] ${isCurrentUser ? 'order-2' : 'order-1'}`}>
                  ...
                  <div className={`rounded-lg p-3 ${isCurrentUser ? 'bg-[#F5A623] text-black' : 'bg-[#351470] text-white border border-white/10'}`}>
